...
static C3D_LightLut lut_Phong, lut_Spot;

static void* vbo_data;
static float angleX = 0.0, angleY = 0.0;

static float spot_func(float angle, float a)
{
    if (angle >= 0.99)
        return 1.0f;
    if (angle >= 0.95)
...
static void sceneInit(void)
{
	...
	C3D_FVec lightVec = { { 0.3, 0, 0.4, 1.0 } };

	C3D_LightInit(&light, &lightEnv);
	C3D_LightColor(&light, 1.0, 1.0, 1.0);
	C3D_LightPosition(&light, &lightVec);
	C3D_LightSpotEnable(&light, true);
	C3D_LightSpotDir(&light, -0.6, 0.0, -0.8);
	LightLut_FromFunc(&lut_Spot, spot_func, 0, true);
	//LightLut_Spotlight(&lut_Spot, 0.3);
    C3D_LightSpotLut(&light, &lut_Spot);
}
